...
- **Causa 2** (sin rate limit en `/api/admin/auth`): cerrado 2026-04-19. Implementado con Upstash Redis (`@upstash/ratelimit` + `@upstash/redis`), `fixedWindow(5, '15 m')` por IP → HTTP 429 con `Retry-After`. `lib/rate-limit.ts` maneja el backend; `app/[locale]/admin/panel.tsx` distingue 401 vs 429 con countdown MM:SS en el botón.
  - **Historial:** primer intento fue Supabase-backed (tabla `admin_login_attempts`, migración 006) — revertido porque el fail-open enmascaró que la migración nunca se ejecutó. Segundo ciclo: switch a Upstash (recomendación original del audit). Tercer ciclo: fix de frontend que mostraba "Credenciales incorrectas" en cualquier error y enmascaraba el 429.
  - **Smoke test producción (2026-04-19):** 6 intentos rápidos con password inválida → el 6to muestra countdown; Upstash dashboard registra bloqueos. Funciona end-to-end.

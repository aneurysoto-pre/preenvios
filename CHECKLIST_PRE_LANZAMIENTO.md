...
- [ ] **Formalizar aplicaciones a programas de afiliado** (estado al 2026-04-23 en Proceso 28 sección "Validación de supuestos"). Ya hay evidencia empírica: CJ pre-aprobado, Impact + FlexOffers con conversaciones abiertas con agentes directos. Falta: completar aplicaciones formales ahora que el sitio está más preparado (cookie consent, JSON-LD, DB separada, Agente 1 operativo), y documentar en el Proceso 28 qué entrega cada programa exactamente (deeplink / feed / pricing API / combo) según las respuestas de los agentes. Si alguna red no da pricing API para un operador, activar Fase R0 (puente manual) para ese operador hasta que R2/R3 lo cubra.
- [ ] **Decisión explícita de go/no-go el día del cutover — 4 escenarios posibles:**
  - **Escenario A (ideal):** Fase R1 ejecutada (Wise API oficial, 1/7 operador funcional) + Fase R2 parcial (al menos 2 partners más aprobados y con API integrada) → lanzar con 3-7/7 operadores con data automatizada. Sin banner necesario si cubre 7/7; con banner "actualizada diariamente" en los operadores restantes bajo Fase R0.
  - **Escenario A' (puente manual — filosofía "LANZA BIEN, NO LANZES RÁPIDO"):** Fase R1 activa (Wise) + Fase R0 (founder actualiza manualmente 7/7 operadores, 2h/día) durante las primeras 2-4 semanas post-launch hasta que R2 cubra más operadores. Banner de transparencia visible indicando "tasas actualizadas diariamente por equipo editorial". Es la vía más realista si R2 no avanzó al ritmo esperado pero se decide lanzar igual con calidad de data humana verificada.
  - **Escenario B (pausar cutover):** R1 no ejecutada + R2 sin aprobaciones + founder NO puede sostener R0 (2h/día por 2-4 semanas) → NO activar DNS. Re-agendar cutover. Enfocar esfuerzo en cerrar aprobaciones de afiliados o ejecutar R1 (quick win gratis).
  - **Escenario C (descartado):** lanzar con data estática placeholder sin R0 ni R1 ni R2. Rompe la propuesta de valor. No debería usarse — el fallback real es R0 manual.
